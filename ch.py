...
#  piks
# Version: 1.3.8
# WARNING! this module not an original, some function has been edited by: peroRIST
# Description:
...
def _parseFont(f):
  ...
  #' x22123456="1"'['1470370466.75', 'peroRIST', '', '71655925', '2CBfuZCZPZWmjEjFsFqJYA==', 'V6zFfp-P8evHoLle9I_Pzw==', '180.244.164.15', '12', '', '<n60c/><f x22123456="1">nyan']
  try: #TODO: remove quick hack
    f = f.strip()
    sizecolor, face = f.split("=", 1)
    if sizecolor[1:3].isnumeric():
      size = int(sizecolor[1:3])
      try: color = sizecolor[3:]
      except: color = None
    else:
      size = None
      try: color = sizecolor[1:]
      except: color = None
    face = face.replace('"', '')
    if face == "": face = None  
...
class _ANON_PM_OBJECT:
  ...
  def message(self, user, msg):
    """send a pm to a user"""
    if msg!=None:
      self._sendCommand("msg", user.name.lower(), msg)
  ...
  def _process(self, data):
    ...
    if hasattr(self, func):
      ...
      if debug  and data[0] != "":
        print("unknown data [ANON_PM]: "+str(data))
...
class ANON_PM:
  ...
  def message(self, user, msg):
    """send a pm to a user"""
    if not user.name.lower() in self._persons:
      self._connect(user.name.lower())
    self._persons[user.name.lower()].message(user,msg)
...
class PM:
  ...
  def _process(self, data):
    ...
    if hasattr(self, func):
      ...
      if debug and data[0] != "":
        print("unknown data [PM]: "+str(data))
  ...
  def _rcmd_idleupdate(self, args):
    ...
    if args[1] == '1':
      try: self._status[user] = [last_on, is_on, 0]
      except: pass
    else:
  ...
  def addContact(self, user):
    ...
    if user not in self._contacts:
      self._sendCommand("wladd", user.name.lower())
      self._contacts.add(user)
  ...
  def removeContact(self, user):
    ...
    if user in self._contacts:
      self._sendCommand("wldelete", user.name.lower())
  ...
  def block(self, user):
    ...
    if user not in self._blocklist:
      self._sendCommand("block", user.name.lower(), user.name.lower(), "S")
  ...
  def unblock(self, user):
    """unblock a person"""
    if user in self._blocklist:
      self._sendCommand("unblock", user.name.lower())

  def track(self, user):
    """get and store status of person for future use"""
    self._sendCommand("track", user.name.lower())
...
class Room:
  ...
  def _getManager(self): return self._mgr
  def _getUserlist(self, mode = Userlist_All, unique = None, memory = None):
    ul = None
  ...
  def _process(self, data):
    ...
    if hasattr(self, func):
      ...
      if debug and data[0] != "":
        print("unknown data [Room]: "+str(data))
  ...
  def _rcmd_unblocked(self, args):
    ...
    target = User(args[2])
    user=User(args[3])
    try: del self._banlist[target]
    except: pass
    try: self._unbanlist[user] = {"unid":args[0], "ip":args[1], "target":target, "time":float(args[4]), "src":user}
    except: pass
  ...
  def message(self, msg, html = False):
    ...
    if self._currentname != None and not self._currentname.startswith("!anon"):
      font_properties = "<n%s/><f x%0.2i%s=\"%s\">" %(self.user.nameColor, self.user.fontSize, self.user.fontColor, self.user.fontFace)
      if "\n" in msg:
  ...
  def addMod(self, user):
    ...
    if self.getLevel(User(self.currentname)) > 0:
      self._sendCommand("addmod", user.name.lower())
  ...
  def removeMod(self, user):
    ...
    """
    if self.getLevel(User(self.currentname)) > 0:
      self._sendCommand("removemod", user.name.lower())
  ...
  def clearUser(self, user):
    ...
    if self.getLevel(self.user) > 0:
      msg = self.getLastMessage(user)
      if msg:
        if msg.user.name.lower()[0] in ["!","#"]:self.rawClearUser(msg.unid, msg.ip,"")
        else:self.rawClearUser(msg.unid,msg.ip,msg.user.name.lower())
        return True
    return False
  ...
  def ban(self, msg):
    ...
    """
    if self.getLevel(self.user) > 0:
      self.rawBan(msg.user.name.lower(), msg.ip, msg.unid)
  ...
  def getLevel(self, user):
    ...
    if user.name.lower() == self.ownername: return 2
    if user.name.lower() in self.modnames: return 1
    return 0
...
class RoomManager:
  ...
  def getName(self, name):
    for un in _users.keys():
      if name.lower() == un.lower(): return un
    return name
  ####
  # Properties
  ####
  ...
  def _getUsers(self): return User
...
def getUser(name):
  for k in _users.keys():
    if name.lower() == k.lower(): return k
  return name.lower()
